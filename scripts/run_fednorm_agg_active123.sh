#!/bin/bash
# FedNorm加入DGA（agg_factors控制DGA的权重）
# active_rates=(0.1 0.2 0.3)
active_rates=(0.3)
seeds=(555)
# agg_factors=(1.0 0.2 0.4 0.6 0.8 1.2 1.4 1.6 1.8 2.0)
agg_factors=(0.4 0.6 0.8 1.2 1.4 1.6 1.8 2.0)


log_file="script/fednorm_agg_active123.log"
...
for active_rate in "${active_rates[@]}"; do
    for seed in "${seeds[@]}"; do
        for agg_factor in "${agg_factors[@]}"; do
            # baseline
            baseline_train="CUDA_VISIBLE_DEVICES=2 python train_fedavg.py --num_clients 10 --active_rate $active_rate --num_rounds 20 --num_epochs 2 --batch_size 8 --lr_decay -1 --alpha 5.5 --dataset both --log_dir logs_baseline_active123 --tag fednorm_agg${agg_factor}_active${active_rate}_seed${seed} --seed ${seed} --agg_strategy --agg_factor ${agg_factor}"
            baseline_eval="CUDA_VISIBLE_DEVICES=2 python eval_central.py --batch_size 8 --threshold 8e-3 --dataset both --log_dir logs_baseline_active123 --tag fednorm_agg${agg_factor}_active${active_rate}_seed${seed} --mode fed"
