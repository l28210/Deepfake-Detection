...
def heatmap(data, camp='RdYlGn', figsize=(10, 10.75), ax=None, save_path=None):
    fig, ax = plt.subplots(figsize=figsize, dpi=40)
    ...
    # 使用 matplotlib 的 colorbar 并指定图像对象
    cb = plt.colorbar(sns_heatmap.collections[0], cax=cax, orientation='horizontal')
    cax.xaxis.set_ticks_position('top')

    if save_path:
        plt.savefig(save_path)
    plt.close(fig)  # 防止显示多张图
