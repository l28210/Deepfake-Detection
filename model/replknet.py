...
def get_conv2d(in_channels, out_channels, kernel_size, stride, padding, dilation, groups, bias):
    ...
    if has_large_impl and in_channels == out_channels and out_channels == groups and use_large_impl and stride == 1 and padding == kernel_size // 2 and dilation == 1:
        # 使用大卷积核时调用高性能计算库取代参数重优化部分,否则使用参数重优化
        ...
        # print('using DepthWiseConv2dImplicitGEMM')
...
def print_num_model(model):
    total_params = sum(p.numel() for p in model.parameters())
    print(f"Total parameters: {total_params}")

if __name__ == '__main__':
    # os.environ['LARGE_KERNEL_CONV_IMPL'] = '/home/l/test_self/deepfake_detect/lib/RepLKNet-pytorch/cutlass/examples/19_large_depthwise_conv2d_torch_extension/'
    os.environ['LARGE_KERNEL_CONV_IMPL'] = 'lib/RepLKNet-pytorch/cutlass/examples/19_large_depthwise_conv2d_torch_extension/'

    model = create_RepLKNet31B(small_kernel_merged=False,num_classes=1)
    model.eval()
    print('------------------- training-time model -------------')
    # print(model)
    # 查看总参数数量
    print_num_model(model)
    # x = torch.randn(2, 3, 224, 224)
    x = torch.randn(2, 3, 256, 256)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model.to(device)
    x = x.to(device)  # 确保输入张量也在同一个设备上
    print(x.shape)
    origin_y = model(x)
    # 若使用高性能计算库则无需优化参数结构
